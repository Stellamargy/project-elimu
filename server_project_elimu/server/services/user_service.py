...
class UserService:
    ...
    def validate_unique_fields(data):
        ...
        if User.query.filter_by(email=data.get("email")).first():
            return error_response(
                status="error", message="Email already exist", status_code=409
            )
        # unique national id - NB:allows null value
        if data.get("national_identification_number"):
            if User.query.filter_by(
                national_identification_number=data.get(
        ...
        if Student.query.filter_by(student_number=data.get("student_number")).first():
            return error_response(
                status="error", message="Student number already exist", status_code=409
            )
        ...
        if Instructor.query.filter_by(
            ...
            return error_response(
                status="error", message="Employee Number already exist", status_code=409
            )
    ...
    def is_creator_id_valid(data):
        if not User.query.get(data.get("created_by")):
            return error_response(
                status="error", message="Creator does not exist", status_code=404
            )
    ...
    def register_user(cls, data):
        if not data.get("role_id"):
            return error_response(
                status="error", message="Role id is required", status_code=400
            )

        role = Role.query.get(data.get("role_id"))
        if not role:
            return error_response(
                status="error", message="Invalid role_id", status_code=400
            )

        role_name = role.role_name.lower()
        if role_name not in cls.user_class_map:
            return error_response(
                status="error", message="Role not supported", status_code=404
            )
        ...
        try:
            ...
            db.session.rollback()
            return error_response(
                status="error", message="A database error occurred.", status_code=500
            )
